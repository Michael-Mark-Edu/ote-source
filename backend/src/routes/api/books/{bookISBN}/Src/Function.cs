...
public class Function
{
    ...
    public async Task<APIGatewayHttpApiV2ProxyResponse> FunctionHandler(APIGatewayHttpApiV2ProxyRequest request, ILambdaContext context)
    {
        ...
        try
        {
            bookISBN = request.PathParameters["bookISBN"];
        }
        catch (NullReferenceException)
        {
            ...
            {
                StatusCode = 400,
                Body = $"{{\"error\":\"bookISBN expected but not given.\"}}",
        ...
        {
            case "GET":
                return await get(request, context, bookISBN);
            case "PATCH":
                return await patch(request, context, bookISBN);
            case "DELETE":
                return await delete(request, context, bookISBN);
            default:
                return new APIGatewayHttpApiV2ProxyResponse
                {
                    StatusCode = 405,
    ...
    private async Task<APIGatewayHttpApiV2ProxyResponse> get(APIGatewayHttpApiV2ProxyRequest request, ILambdaContext context, string bookISBN)
    {
        using var oteContext = new OteContext();

        var foundBook = await oteContext
            .Books
            .Where(e => e.ISBN == bookISBN)
        ...
        if (foundBook == null)
            ...
            {
                StatusCode = 404,
                Body = $"{{\"error\":\"Book with ISBN '{bookISBN}' does not exist.\"}}",
                Headers = new Dictionary<string, string> { { "Content-Type", "application/json" } }
            };

        var bookGetDto = new BookGetDto(foundBook);
    ...
    private async Task<APIGatewayHttpApiV2ProxyResponse> patch(APIGatewayHttpApiV2ProxyRequest request, ILambdaContext context, string bookISBN)
    {
        using var oteContext = new OteContext();
        ...
        var updatedBookEntry = oteContext
            .Books
            .Update(foundBook);
        ...
        var updatedBook = updatedBookEntry.Entity;
        var bookGetDto = new BookGetDto(updatedBook);
        var bookGetDtoJson = JsonSerializer.Serialize(bookGetDto);
    ...
    private async Task<APIGatewayHttpApiV2ProxyResponse> delete(APIGatewayHttpApiV2ProxyRequest request, ILambdaContext context, string bookISBN)
    {
        using var oteContext = new OteContext();
        ...
        foundBook.DeletedAt = DateTime.UtcNow;
